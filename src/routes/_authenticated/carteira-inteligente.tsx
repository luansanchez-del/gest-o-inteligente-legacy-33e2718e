...
export const Route = createFileRoute("/_authenticated/carteira-inteligente")({
  ...
  head: () => ({ meta: [{ title: "Carteira Inteligente | Gestão Inteligente" }] }),
});
...
function canonizarLinha(row: Record<string, unknown>) {
  const honorarioGenerico = valorPorAlias(row, ["honorario atual", "honorarios", "honorario", "fee", "mensalidade"]);
  const valorBpoExplicito = valorPorAlias(row, ["valor bpo", "repasse bpo", "custo bpo", "pagamento bpo", "repasse"]);
  const layoutFinanceiro = temCabecalho(row, ["honorario atual"]) && (temCabecalho(row, ["grupo"]) || temCabecalho(row, ["status"]));
  const layoutCarteiraBpo = !layoutFinanceiro && temCabecalho(row, ["bpo"]);

  return {
    cliente: valorPorAlias(row, ["cliente", "razao social", "razao", "empresa", "nome cliente"]),
    cnpj: valorPorAlias(row, ["cnpj", "cpf", "documento"]),
    status: valorPorAlias(row, ["status", "situacao"]),
    grupo: valorPorAlias(row, ["grupo", "grupo economico", "grupo empresarial"]),
    responsavel: valorPorAlias(row, ["responsavel", "bpo responsavel", "carteira", "contador", "bpo"]),
    regime: valorPorAlias(row, ["regime", "tributacao", "regime tributario"]),
    segmento: valorPorAlias(row, ["segmento", "atividade", "ramo"]),
    honorario: layoutCarteiraBpo && valorBpoExplicito === undefined ? undefined : honorarioGenerico,
    valorBpo: layoutFinanceiro ? undefined : valorBpoExplicito ?? (layoutCarteiraBpo ? honorarioGenerico : undefined),
    peso: valorPorAlias(row, ["peso", "complexidade", "pontos", "carga"]),
    observacao: valorPorAlias(row, ["observacao", "obs", "comentario", "nota"]),
    tipoImportacao: layoutFinanceiro ? "HONORARIOS_GRUPOS" : "CARTEIRA_BPO",
  };
}
...
function linhasPlanilha(ws: XLSX.WorkSheet) {
  ...
  for (let col = 0; col < maxCols; col++) {
    ...
    for (let row = 0; row < headerIndex; row++) {
      if (normalizar(matriz[row]?.[col]) === "bpo") { headers[col] = "BPO"; break; }
    }
  }
  return matriz.slice(headerIndex + 1).map((values) => {
    const row: Record<string, unknown> = {};
    headers.forEach((header, col) => { if (header) row[header] = values?.[col] ?? ""; });
    return row;
...
function CarteiraInteligentePage() {
  ...
  const [perfil, setPerfil] = useState({ nome: "", email: "", senioridade: "", capacidade: "60", regimes: "", segmentos: "", sistemas: "", competencias: "", curriculo: "" });

  const consulta = useQuery({ queryKey: ["carteira-inteligente"], queryFn: () => listarCarteiraInteligente() });

  const importar = useMutation({
    mutationFn: () => importarCarteiraInteligente({ data: { rows: linhasImportacao } }),
    onSuccess: (r) => {
      toast.success(`${r.importadas} clientes ativos atualizados.`);
      if (r.ignoradasInativas) toast.info(`${r.ignoradasInativas} linha(s) inativa(s) ignorada(s).`);
      if (r.falhas.length) toast.warning(`${r.falhas.length} linha(s) não foram importadas.`);
      setImportOpen(false); setLinhasImportacao([]); setArquivoNome("");
  ...
  const perfisPier = useMutation({
    ...
    onSuccess: (r) => { toast.success(`${r.total} profissionais ativos do PIER disponíveis.`); void qc.invalidateQueries({ queryKey: ["carteira-inteligente"] }); },
    onError: (e) => toast.error(mensagemDeErro(e)),
  });

  const salvarPerfil = useMutation({
    mutationFn: () => salvarPerfilBpo({ data: {
      nome: perfil.nome, email: perfil.email || null, senioridade: perfil.senioridade || null,
      capacidade: Number(perfil.capacidade || 60), regimes: separar(perfil.regimes), segmentos: separar(perfil.segmentos),
      sistemas: separar(perfil.sistemas), competencias: separar(perfil.competencias), curriculoTexto: perfil.curriculo || null,
    } }),
    onSuccess: () => { toast.success("Perfil profissional salvo."); setPerfilOpen(false); void qc.invalidateQueries({ queryKey: ["carteira-inteligente"] }); },
    onError: (e) => toast.error(mensagemDeErro(e)),
  });
  ...
  const sugestao = useQuery({
    ...
    enabled: Boolean(clienteSugestao), staleTime: 0,
  });

  const atribuir = useMutation({
    mutationFn: (profileId: string) => atribuirClienteCarteira({ data: { clientKey: clienteSugestao!, profileId } }),
    onSuccess: () => { toast.success("Responsável definido na Carteira Inteligente. O PIER não foi alterado."); setClienteSugestao(null); void qc.invalidateQueries({ queryKey: ["carteira-inteligente"] }); },
    onError: (e) => toast.error(mensagemDeErro(e)),
  });
  ...
  const linhas = useMemo(() => {
    ...
    if (q) xs = xs.filter((x) => normalizar(`${x.nome} ${x.documento ?? ""} ${x.grupo ?? ""} ${x.responsavelPier ?? ""} ${x.responsavelCarteira ?? ""}`).includes(q));
    return xs;
  ...
  async function carregarArquivo(file: File) {
    try {
      ...
      const canonicas = linhasPlanilha(ws)
        .map(canonizarLinha)
        .filter((r) => r.cliente || r.cnpj)
        .filter((r) => !r.status || normalizar(r.status) === "ativo");
      setLinhasImportacao(canonicas); setArquivoNome(file.name);
      if (!canonicas.length) toast.error("Não encontrei clientes ativos na primeira aba.");
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="distribuicao" className="space-y-4">
      ...
      <TabsContent value="distribuicao" className="space-y-4">
        <Card className="p-4"><div className="flex flex-col gap-3 md:flex-row">
          <Input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar cliente, CNPJ, grupo ou responsável" className="md:max-w-md" />
          <Select value={filtro} onValueChange={(v) => setFiltro(v as Filtro)}><SelectTrigger className="md:w-56"><SelectValue /></SelectTrigger><SelectContent>
            <SelectItem value="TODOS">Todos os ativos</SelectItem><SelectItem value="SEM_CARTEIRA">Somente sem carteira</SelectItem><SelectItem value="DIVERGENCIA">PIER x carteira divergente</SelectItem>
        ...
        <Card className="overflow-hidden">
          {consulta.isLoading ? <div className="p-8 text-sm text-muted-foreground">Carregando clientes ativos…</div> : !linhas.length ? <EstadoVazio titulo="Nenhum cliente neste filtro." descricao="Sincronize o PIER ou importe a planilha financeira." /> : <Table>
            ...
            <TableBody>{linhas.map((l) => <TableRow key={l.clientKey}>
              ...
              <TableCell>{l.grupo ? <div><p className="max-w-48 text-sm">{l.grupo}</p>{l.honorarioCobertoPorGrupo ? <Badge className="mt-1" variant="secondary">Incluso no grupo</Badge> : null}</div> : <span className="text-muted-foreground">Sem grupo</span>}</TableCell>
    ...
    <Dialog open={perfilOpen} onOpenChange={setPerfilOpen}><DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>Perfil profissional BPO</DialogTitle><DialogDescription>Use somente informações profissionais relevantes para a distribuição.</DialogDescription></DialogHeader>
      <div className="grid gap-4 md:grid-cols-2"><Campo label="Nome" value={perfil.nome} onChange={(v) => setPerfil((p) => ({ ...p, nome: v }))} /><Campo label="E-mail" value={perfil.email} onChange={(v) => setPerfil((p) => ({ ...p, email: v }))} /><Campo label="Senioridade" value={perfil.senioridade} onChange={(v) => setPerfil((p) => ({ ...p, senioridade: v }))} /><Campo label="Capacidade em pontos" value={perfil.capacidade} onChange={(v) => setPerfil((p) => ({ ...p, capacidade: v }))} /><Campo label="Regimes tributários" value={perfil.regimes} onChange={(v) => setPerfil((p) => ({ ...p, regimes: v }))} /><Campo label="Segmentos" value={perfil.segmentos} onChange={(v) => setPerfil((p) => ({ ...p, segmentos: v }))} /><Campo label="Sistemas" value={perfil.sistemas} onChange={(v) => setPerfil((p) => ({ ...p, sistemas: v }))} /><Campo label="Competências" value={perfil.competencias} onChange={(v) => setPerfil((p) => ({ ...p, competencias: v }))} /></div>
      <div className="space-y-2"><Label>Currículo / histórico profissional</Label><Textarea rows={8} value={perfil.curriculo} onChange={(e) => setPerfil((p) => ({ ...p, curriculo: e.target.value }))} /></div>
