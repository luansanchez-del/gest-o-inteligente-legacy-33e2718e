...
function classeAcao(acao: string) {
  if (acao === "RESPONDER_FINALIZAR") return "bg-success-soft text-success-strong";
...
function MinhasSolicitacoesPage() {
  ...
  const sincronizar = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(
        `${r.processadas} solicitações atualizadas para ${r.usuario.nome}.${r.possivelmenteParcial ? " A API atingiu o limite de páginas; refine e sincronize novamente se necessário." : ""}`,
      );
  ...
  useEffect(() => {
    if (analise.data?.respostaSugerida) setMensagem(analise.data.respostaSugerida);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        acoes={
          ...
          >
            ...
            />
            {sincronizar.isPending ? "Sincronizando…" : "Sincronizar minhas solicitações"}
          </Button>
        }
      />
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">atribuídas a {dados?.usuario.nome ?? "você"}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs uppercase tracking-wide text-muted-foreground">Vencidas</p>
          <p className="mt-1 text-3xl font-semibold tabular-nums text-destructive">{dados?.vencidas ?? 0}</p>
        ...
        <Card className="p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">sem decisão automática por prazo</p>
        </Card>
        <Card className="p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">evidências disponíveis para leitura</p>
        </Card>
      </div>
          ...
          <div className="min-w-0 flex-1 space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Ex.: balancete, nome do cliente, número..."
                className="pl-9"
              />
            </div>
          </div>
      ...
      <Card className="overflow-hidden">
        {caixa.isLoading ? (
          <div className="p-8 text-sm text-muted-foreground">Carregando sua Caixa de Entrada…</div>
        ) : !dados?.linhas.length ? (
          <EstadoVazio
            ...
            <TableBody>
              {dados.linhas.map((linha) => (
                <TableRow key={linha.externalId}>
                  ...
                  <TableCell>
                    <p className="max-w-[260px] truncate font-medium">{linha.clienteNome ?? "Cliente não identificado"}</p>
                  ...
                  <TableCell>
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">Confiança {linha.confianca.toLowerCase()}</p>
                  </TableCell>
                  <TableCell>
                    <p className="max-w-[380px] line-clamp-2 text-sm">{linha.descricao ?? linha.tipo ?? "Sem descrição"}</p>
                    {linha.possuiAnexo ? (
                  ...
                  <TableCell>
                    ...
                    {linha.venceHoje ? <p className="text-xs text-warning-strong">vence hoje</p> : null}
                    {linha.vencida ? <p className="text-xs text-destructive">vencida</p> : null}
                  </TableCell>
      ...
      >
        <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              O sistema localiza contexto e evidências. Nenhuma resposta ou finalização ocorre sem seu comando.
            </DialogDescription>
          </DialogHeader>

          {analise.isLoading ? (
            <div className="p-8 text-center text-sm text-muted-foreground">Lendo PIER, histórico e evidências…</div>
          ) : analise.isError ? (
            ...
            <div className="space-y-5">
              <div className="grid gap-3 md:grid-cols-4">
                <Card className="p-3">
                  ...
                  <p className="mt-1 font-medium">{ROTULOS[analise.data.leitura.categoria] ?? analise.data.leitura.categoria}</p>
                  <p className="text-xs text-muted-foreground">Confiança {analise.data.leitura.confianca.toLowerCase()}</p>
                </Card>
                <Card className="p-3">
                  <p className="text-xs text-muted-foreground">Competência</p>
                  <p className="mt-1 font-medium">{analise.data.solicitacao.competencia ?? "Não identificada"}</p>
                </Card>
                <Card className="p-3">
                  <p className="text-xs text-muted-foreground">Responsável atual</p>
                  <p className="mt-1 font-medium">{analise.data.solicitacao.responsavelNome ?? "—"}</p>
                </Card>
                <Card className="p-3">
                  <p className="text-xs text-muted-foreground">Prazo</p>
                  <p className="mt-1 font-medium">{dataPt(analise.data.solicitacao.prazoEm)}</p>
                </Card>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {analise.data.solicitacao.descricao ?? analise.data.solicitacao.tipo ?? "Sem descrição disponível."}
                    </p>
                    <p className="mt-2 text-xs text-muted-foreground">
                      {analise.data.leitura.postagens} postagem(ns) · {analise.data.leitura.arquivos.length} arquivo(s) lido(s) no contexto
                    </p>
                  </div>
                </div>
              </Card>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <Card className="p-4">
                  ...
                  {analise.data.localizador.responsavelSugerido ? (
                    <div className="mt-3">
                      <p className="font-medium">{analise.data.localizador.responsavelSugerido.nome ?? "Usuário identificado"}</p>
                      <p className="text-sm text-muted-foreground">
                        {analise.data.localizador.responsavelSugerido.departamento ?? "Departamento não identificado"}
                      </p>
                    </div>
                  ) : (
                    <p className="mt-3 text-sm text-muted-foreground">Nenhum responsável alternativo foi identificado com segurança no histórico.</p>
                  )}
                  {!analise.data.localizador.encaminhamentoDisponivel ? (
                    <p className="mt-3 rounded-md bg-muted p-2 text-xs text-muted-foreground">
                      Encaminhamento automático ainda indisponível: {analise.data.localizador.motivoEncaminhamentoIndisponivel}
                    </p>
                  ) : null}
                </Card>
                  ...
                  {analise.data.localizador.fechamento ? (
                    <div className="mt-3 space-y-2 text-sm">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Badge variant="secondary">{analise.data.localizador.fechamento.status}</Badge>
                      </div>
                      <div className="flex items-center justify-between gap-3">
                        <span>Balancete localizado</span>
                        <strong>{analise.data.localizador.fechamento.balanceteLocalizado ? "Sim" : "Não"}</strong>
                      </div>
                      <div className="flex items-center justify-between gap-3">
                        <span>Conciliação</span>
                        <strong>
                          {analise.data.localizador.fechamento.conciliacao === "CONFIRMADA_POR_EVIDENCIA"
                    ...
                    <p className="mt-3 text-sm text-muted-foreground">Não foi localizado um fechamento contábil correspondente ao cliente e à competência identificada.</p>
                  )}
                </Card>
              </div>
              ...
              <Card className="border-primary/20 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <span className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${classeAcao(analise.data.recomendacao.acao)}`}>
                      {rotuloAcao(analise.data.recomendacao.acao)}
                    </span>
                    <p className="mt-2 text-sm text-muted-foreground">{analise.data.recomendacao.motivo}</p>
                  </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Por padrão a resposta é visível ao solicitante. Marque como privada somente quando a mensagem for interna.
                </p>
              </div>

              {confirmarFinalizacao ? (
                <Card className="border-warning/40 bg-warning-soft p-4">
                  <p className="font-medium text-warning-strong">Confirmar resposta e finalização?</p>
                  ...
                  <div className="mt-3 flex justify-end gap-2">
                    <Button variant="ghost" onClick={() => setConfirmarFinalizacao(false)} disabled={executar.isPending}>
                      Cancelar
                    </Button>
                    <Button onClick={() => executar.mutate("RESPONDER_FINALIZAR")} disabled={executar.isPending || mensagem.trim().length < 10}>
          ...
          <DialogFooter className="flex-col gap-2 sm:flex-row">
            <Button
              ...
              disabled={executar.isPending || mensagem.trim().length < 10 || !analise.data}
            >
