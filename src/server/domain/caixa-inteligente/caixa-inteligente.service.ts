...
function dataSomente(value: string | null | undefined) {
...
function classificar(texto: string): {
  ...
  if (/balancete|balanco|razao|dre|demonstracao do resultado/.test(t))
  ...
  if (/folha|ferias|13[ºo]?|decimo terceiro|pro.?labore|rescis|admiss|esocial|e-social/.test(t))
    return { categoria: "FOLHA", confianca: "ALTA" };
  if (/pagamento|recebimento|financeiro|boleto|cobranca|extrato bancario|banco|pix/.test(t))
...
function termosDepartamento(categoria: CategoriaSolicitacao) {
  if (categoria === "FISCAL") return ["fiscal", "tribut"];
  if (categoria === "FOLHA") return ["folha", "departamento pessoal", "dp"];
  if (categoria === "FINANCEIRO") return ["financeiro"];
  if (categoria === "BALANCETE" || categoria === "CONTABIL")
    return ["contabil", "contabilidade"];
  return [];
}
...
async function resolverUsuarioPier(ctx: AppContext, email?: string) {
  if (!email)
    throw new AppError(
      "REGRA_NEGOCIO",
      "Seu login não possui e-mail disponível para localizar o usuário correspondente no PIER.",
    );
  ...
  const { data: cache } = await ctx.db
    .from("pier_user")
    .select(
      "external_id, name, email, login, department_external_id, status, kind",
    )
    .eq("organization_id", ctx.organizationId);

  const encontradoCache = (cache ?? []).find(
    (u) =>
      normalizar(u.email) === normalizar(alvo) ||
      normalizar(u.login) === normalizar(alvo),
  );
  if (encontradoCache)
    return {
      externalId: encontradoCache.external_id,
      name: encontradoCache.name,
      email: encontradoCache.email,
      login: encontradoCache.login,
      departmentExternalId: encontradoCache.department_external_id,
      status: encontradoCache.status,
      kind: encontradoCache.kind,
    };

  const usuarios = await pierAdapter.listUsers({ status: "Ativo" });
  ...
  if (!encontrado)
    throw new AppError(
      "REGRA_NEGOCIO",
      `Não foi possível vincular o login ${email} a um usuário ativo do PIER. Sincronize a equipe ou confira o e-mail/login cadastrado no PIER.`,
    );
  return encontrado;
}
...
async function gravarSolicitacoes(
  ...
  usuario: Pick<PierUser, "departmentExternalId">,
) {
  ...
  for (let inicio = 0; inicio < solicitacoes.length; inicio += 250) {
    ...
    const { error } = await ctx.db.from("request").upsert(
      lote.map((s) => ({
        ...
        department_external_id: usuario.departmentExternalId,
...
export async function sincronizarMinhaCaixa(
  ...
  try {
    solicitacoes = await pierAdapter.listRequests({
  ...
  const minhas = solicitacoes.filter(
    (s) =>
      s.responsibleExternalId === usuario.externalId &&
      !solicitacaoFinalizadaPier(s.status, s.finishedAt),
  );
  const processados = await gravarSolicitacoes(ctx, minhas, usuario);
  const possivelmenteParcial =
    solicitacoes.length >= (statusConsultado === "Andamento" ? 60 : 40) * 30;

  await audit(ctx, {
    action: "SINCRONIZAR_CAIXA_INTELIGENTE",
    entity: "request",
    after: {
      usuarioPier: usuario.externalId,
  ...
  return {
    usuario: { id: usuario.externalId, nome: usuario.name },
    encontradas: minhas.length,
    processadas: processados,
    possivelmenteParcial,
  };
...
export async function listarMinhaCaixa(
  ...
  const { data, error } = await ctx.db
    .from("request")
    .select(
      "id, external_id, number, description, type_name, purpose, reference_month, status, responsible_external_id, responsible_name, client_name, client_document, requested_at, deadline_at, finished_at, has_attachment, synced_at",
    )
    .eq("organization_id", ctx.organizationId)
    .eq("responsible_external_id", usuario.externalId)
  ...
  const linhas = (data ?? [])
    ...
    .map((r) => {
      ...
      const prazo = dataSomente(r.deadline_at);
      return {
        id: r.id,
    ...
    .filter((r) => {
      if (input.categoria && input.categoria !== "TODAS") {
        if (r.categoria !== input.categoria) return false;
      }
...
async function localizarHistoricoResponsavel(
  ctx: AppContext,
  ...
  let query = ctx.db
    .from("request")
    .select(
      "external_id, type_name, description, responsible_external_id, responsible_name, department_external_id, finished_at, status",
    )
    .eq("organization_id", ctx.organizationId)
    .not("responsible_external_id", "is", null)
    .limit(100);
  if (solicitacao.clientDocument)
    query = query.eq("client_document", solicitacao.clientDocument);
  else if (solicitacao.clientExternalId)
    query = query.eq("client_external_id", solicitacao.clientExternalId);
  else return null;

  const { data } = await query;
  const termos = termosDepartamento(categoria);
  ...
  for (const r of data ?? []) {
    ...
    let pontos = 1;
    if (termos.some((termo) => texto.includes(termo))) pontos += 4;
    if (solicitacao.responsibleExternalId === r.responsible_external_id) pontos -= 2;
    const atual = pontuacao.get(r.responsible_external_id);
    if (atual) atual.pontos += pontos;
    else
      pontuacao.set(r.responsible_external_id, {
...
async function localizarFechamento(
  ...
  let query = ctx.db
    .from("request")
    .select(
      "id, external_id, number, description, type_name, purpose, status, finished_at, responsible_name, reference_month",
    )
    .eq("organization_id", ctx.organizationId)
  ...
  const [{ data: processamento }, { data: execucoes }, { data: anexos }] =
    await Promise.all([
      ctx.db
        .from("request_processing")
        .select("outcome, reason, finalized_at, pier_status, updated_at")
        .eq("organization_id", ctx.organizationId)
  ...
  let arquivosPier: { name: string | null; externalId: string }[] = [];
  try {
    arquivosPier = (await pierAdapter.listFiles({
      requestExternalId: fechamento.external_id,
  ...
  const nomes = [
    ...(anexos ?? []).map((a) => a.filename),
    ...arquivosPier.map((a) => a.name ?? ""),
  ].filter(Boolean);
  const balancetes = nomes.filter((n) => /balancete|balanco/i.test(n));
  const execucao = (execucoes ?? [])[0] ?? null;
  const textoEvidencia = normalizar(
    `${textos} ${fechamento.description ?? ""} ${JSON.stringify(execucao?.summary ?? {})} ${JSON.stringify(execucao?.result ?? {})}`,
  );
  const conciliacaoConfirmada =
    /conciliad|conciliacao concluida|conciliacao finalizada/.test(textoEvidencia);
  const concluido =
...
export async function analisarSolicitacao(
  ...
  if (solicitacao.responsibleExternalId !== usuario.externalId)
    throw new AppError(
      "REGRA_NEGOCIO",
  ...
  const textos = [
    ...
    .filter(Boolean)
    .join("\n");
  const classificacao = classificar(textos);
  const competencia = solicitacao.referenceMonth ?? extrairCompetencia(textos);
  const [responsavelSugerido, fechamento] = await Promise.all([
    localizarHistoricoResponsavel(ctx, solicitacao, classificacao.categoria),
    localizarFechamento(ctx, solicitacao, competencia, textos),
  ]);

  const temOutroResponsavel =
    responsavelSugerido && responsavelSugerido.id !== usuario.externalId;
  let acao: AcaoRecomendada = "REVISAO_HUMANA";
  let motivo = "A solicitação precisa de leitura humana antes de qualquer ação no PIER.";
  ...
  if (
    ...
    motivo =
      "O fechamento foi localizado como concluído, há balancete disponível e existe evidência textual de conciliação.";
  } else if (temOutroResponsavel) {
    acao = "ENCAMINHAR";
...
export async function executarAcao(
  ...
  if (atual.responsibleExternalId !== usuario.externalId)
    throw new AppError(
      "REGRA_NEGOCIO",
  ...
  if (solicitacaoFinalizadaPier(atual.status, atual.finishedAt))
    throw new AppError(
      "REGRA_NEGOCIO",
      "A solicitação já está finalizada no PIER.",
    );
  ...
  if (!postagem.externalId)
    throw new AppError(
      "INTEGRACAO",
      "O PIER recebeu a postagem, mas não confirmou o identificador. A solicitação não será finalizada automaticamente.",
    );
